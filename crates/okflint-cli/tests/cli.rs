...
fn bin() -> Command {
    Command::new(env!("CARGO_BIN_EXE_okflint"))
}
...
/// okflint's own documentation is an OKF bundle; it must stay conformant and
/// lint-clean (the dogfood).
...
fn tempdir() -> PathBuf {
    let dir = std::env::temp_dir().join(format!("okflint-cli-test-{}", std::process::id()));
