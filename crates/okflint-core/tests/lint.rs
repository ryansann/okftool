...
use okflint_core::{build_bundle, check, lint, rule_metas, ResolvedConfig, Severity};

fn bundle_with(files: Vec<(&str, &str)>) -> okflint_core::Bundle {
    build_bundle(files)
}

fn codes(diags: &[okflint_core::Diagnostic]) -> Vec<&str> {
